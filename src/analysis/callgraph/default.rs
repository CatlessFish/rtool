...
use crate::{
    analysis::Analysis,
    analysis::callgraph::{CallGraph, CallGraphAnalysis},
    rtool_debug, rtool_info,
};
        ...
        for local_def_id in self.tcx.hir_body_owners() {
            ...
            if self.tcx.is_mir_available(def_id) {
                ...
                let body: &Body<'_> = match def_kind {
                    ...
                    _ => {
                        rtool_debug!("Skipping def_id {:?} with kind {:?}", def_id, def_kind);
                        continue;
                    }
                };
